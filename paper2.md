...
결과적으로 FAISS-GPU 대비 성능은 낮았으나, **브라우저에서 서버 없이 실행되는 점**, **WASM 대비 6배 이상 빠른 속도**, **높은 정확도 유지**는 WebGPU 기반 아키텍처의 실용성을 입증하였다.

---

## 5. 차별성과 활용 가능성

WebGPU 기반 벡터 DB는 기존 GPU 시스템을 단순히 "브라우저로 옮긴 것"이 아니라, **새로운 형태의 검색 경험과 응용 시나리오를 가능하게 하는 기술적 전환점**이다. 특히 다음과 같은 차별성이 뚜렷하다.
...
필요하시면 이 논문을 LaTeX, Word, 또는 학회 포맷으로도 정리해드릴 수 있습니다. 혹시 다음 단계로 준비하시고 싶은 것이 있을까요?
