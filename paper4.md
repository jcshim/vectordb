...
표 1. QPS, 평균 Latency, Top-1 정확도 측정 실험험
...
이전 실험은 10,000개의 512차원 벡터를 대상으로 하였으나, 실용적 활용을 위해 확장성 평가를 추가로 수행하였다. 벡터 수를 100K, 500K, 1M으로 순차적으로 증가시키고, 성능 변화 및 메모리 사용량을 분석하였다. 특히 IndexedDB를 활용하여 브라우저 내 벡터 데이터셋을 분할 로딩하는 방식으로 메모리 압력을 완화하였다.
