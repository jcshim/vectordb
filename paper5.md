...
실험은 Chrome Canary 브라우저의 WebGPU 환경에서 10,000개의 512차원 벡터를 대상으로 수행되었으며, 비교 대상은 다음과 같다:  
- FAISS (GPU 기반 서버) [1]  
- WASM 기반의 단순 클라이언트 측 구현 [4]  
- 본 연구의 WebGPU 기반 시스템
...
FAISS-GPU 대비 성능은 낮았지만, 브라우저에서 별도 서버 없이 실행된다는 점과 WASM 대비 6배 이상의 속도 향상, 높은 정확도는 WebGPU 기반 아키텍처의 실용성을 입증하였다.

추가적으로, 실용적인 활용 가능성을 평가하기 위해 확장성 실험도 진행하였다. 벡터 수를 100K, 500K, 1M으로 순차적으로 증가시키며 성능 변화와 메모리 사용량을 분석하였다. 이때 IndexedDB를 활용하여 브라우저 내에서 벡터 데이터셋을 분할 로딩하는 방식으로 메모리 압력을 완화하였다 [6].
...
IndexedDB 기반의 벡터 페이징 기법은 메모리 사용을 효과적으로 제어하였으며, 1M 벡터 규모에서도 브라우저 환경 내에서 실시간 검색이 가능함을 확인하였다.
